## Meteorology-Pawnee_03-CalculateETo+SumMonthly+SPEI.R

source(file.path("code", "paths+packages.R"))

## load data
df_daily <- 
  readr::read_csv(file.path("data", "Meteorology-Pawnee_Daily_Clean.csv")) %>% 
...
ggsave(file.path("figures+tables", "Meteorology-Pawnee_03-EToComparison.png"),
...
df_daily %>% 
  ...
  readr::write_csv(file.path("data", "Meteorology-Pawnee_Daily_WithETo.csv"))

# monthly data
df_mo %>% 
  dplyr::rename(prcp_mm = prcp_mm_sum, ETo_mm = ETo_mm_sum, tmax_c = tmax_c_mean, tmin_c = tmin_c_mean) %>% 
  readr::write_csv(file.path("data", "Meteorology-Pawnee_Monthly+SPEI.csv"))
