## Data_Meteorology_03-CalculateETo+SumMonthly+SPEI.R
...
# plot and inspect
ggplot(df_yr, aes(x = Year, y = prcp_mm_sum)) +
  geom_point() +
  stat_smooth()

ggplot(df_yr, aes(x = Year, y = ETo_mm_sum)) +
  geom_point() +
  stat_smooth()
...
spei_ts <- seq(1,24)
for (ts in spei_ts){
  spei_calc <- spei(df_mo$defc_mm, scale = ts)
